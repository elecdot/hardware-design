...
| Gree IR AC TX/RX | `handoff/gree_ir_txrx_hardware_package/` | First integration targets TX-only: `rtl/gree_ir_axi/`, `sim/tb_gree_ir_axi/`, `pynq/ir_ac_demo/`, integrated XDC `ir_pwm=T14`; RX remains standalone validation tooling. | IR-1 source migration skeleton, IR-2 focused module regression, IR-3 package static validation, and IR-4 integrated overlay build are complete. IR-5 integrated PYNQ driver smoke completed one `temp_26` TX with `done=true/error=false`; real AC response still needs operator confirmation before closing IR-5. Teammate completed standalone module testing and confirmed the lab Gree AC responds to the handoff command set. Detailed plan: [ir_ac_integration_plan.md](ir_ac_integration_plan.md). |
