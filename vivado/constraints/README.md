...
Use only one I2C pin-mapping XDC for a given top-level build. For the AXI/PYNQ overlay project, use `axi_i2c_jy901_package.xdc` and keep debug ILA XDC files disabled. `jy901_debug.xdc` is intended for the PL-only hardware debug top and includes optional internal weak pullups on SCL/SDA; external 3.3 V pullups are still recommended for real I2C operation.
